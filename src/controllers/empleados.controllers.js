...
export const actualizarEmpleadopatch = async (req, res) => {
    try {
        ...
        if (result.affectedRows === 0) {
            return res.status(404).json({
                mensaje: `Empleados con ID ${id_empleado} no encontrado.`
            });
        }

        res.status(200).json({
            mensaje: `Empleados con ID ${id_empleado} actualizado correctamente.`
        });
    } catch (error) {
        res.status(500).json({
            mensaje: 'Error al actualizar el empleados.',
            error
        });
    }
};